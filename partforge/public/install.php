...
function step_2()
{
    ...
    if (!check_extension('curl')) {
        $error = true;
    }
    if (!check_extension('gd')) {
        $error = true;   // supposedly this means gd2
    }
    if (!check_extension('mysqli')) {
        $error = true;
    }
    if (!check_extension('json')) {
        $error = true;
    }
    if (!check_extension('mbstring')) {
        $error = true;
    }
...
function getSubmittedAndCheckRequired($paramnames)
{
    ...
    foreach ($paramnames as $paramname) {
        ...
        if (empty($_SESSION['globals'][$paramname])) {
            $success = false;
        }
    }
    return $success;
    ...
    ?>
        ...
        <form method="post" action="install.php?step=3">
            ...
            <p style="margin-top: 10px;">
                <input type="submit" name="submit" value="Continue">  <?php if ($allow_skip) {
                    ?><input type="submit" name="submit" value="Skip"> <?php
                                                                      } ?>
            </p>
        </form>
        <?php
...
function prep_step_4()
{
    ...
    if (is_config_default($_SESSION['globals']['support_email'])) {
        $_SESSION['globals']['support_email'] = 'administrator@example.com';
    }

    $_SESSION['globals']['webmaster_email'] = getInstallParamFromConfig($configtext, 'WEBMASTER_EMAIL');
    if (is_config_default($_SESSION['globals']['webmaster_email'])) {
        $_SESSION['globals']['webmaster_email'] = 'administrator@example.com';
    }

    $_SESSION['globals']['notices_from_email'] = getInstallParamFromConfig($configtext, 'NOTICES_FROM_EMAIL');
    if (is_config_default($_SESSION['globals']['notices_from_email'])) {
        $_SESSION['globals']['notices_from_email'] = 'administrator@example.com';
    }
}
