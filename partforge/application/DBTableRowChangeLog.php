...
/**
 *
 * PartForge Enterprise Groupware for recording parts and assemblies by serial number
 * and version along with associated test data and comments.
 *
...
class DBTableRowChangeLog extends DBTableRow {
    ...
    static public function getNewQueryForApiOutput()
    {
        ...
        $DBTableRowQuery->setSelectFields("changelog.changelog_id, changelog.changed_on, changelog.user_id, user.login_id, TRIM(CONCAT(user.first_name,' ',user.last_name)) as full_name,
                        ...
                        IF(typecategory.is_user_procedure=1,itemversion.disposition,null) as procedure_disposition,
    ...
    static public function addedItemReference($has_an_itemobject_id, $belongs_to_itemversion_id)
    {
        ...
        if ($IV->getRecordById($belongs_to_itemversion_id)) {
            ...
            $disposition = empty($IV->disposition) ? '' : ' ('.DBTableRowItemVersion::renderDisposition($IV->getFieldType('disposition'), $IV->disposition, false, '').')';
            $text = $IV->part_description.$serial_identifier.$disposition;
