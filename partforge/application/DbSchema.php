...
class DbSchema {   // singleton
    ...
    private function dbFieldTypeToSchemaType($dbfieldtypes) {
        ...
        foreach($dbfieldtypes as $name => $dbfieldtype) {
            ...
            $mytype = $type_parse[0];
            if ($dbfieldtype['Type']=='int(1)') $mytype = 'boolean';
            if ($dbfieldtype['Type']=='longtext') $mytype = 'text';
            if ($dbfieldtype['Key']=='PRI') $mytype = 'id';
            $out[$name] = array(
                'dbtype' => $dbfieldtype,
            ...
            if (isset($type_parse[1]) && $type_parse[1]) $out[$name]['len'] = $type_parse[1];
        }
        return $out;
    }
	...
	if (!isset($this->_primaryindexname[$tablename])) {
	    ...
	    if (!isset($this->_primaryindexname[$tablename])) throw new Exception("primary index not found for table {$tablename} in DbSchema::getPrimaryIndexName()");
	}
    ...
    public function getJoinFieldsAndTables($tablename) {
        if (!isset($this->_join_fields_and_tables[$tablename])) {
            ...
            foreach($this->getJoins($tablename) as $join_name => $join_link) {
                ...
                if (!isset($join_link['mode'])) $this->_join_fields_and_tables[$tablename][$join_name]['mode'] = 'R';
                if (!isset($join_link['options'])) $this->_join_fields_and_tables[$tablename][$join_name]['options'] = array();
            }
        }
    ...
    */
    public function getTreeTableParams($tablename,$parent_index=null) {
        if (!isset($this->_tree_table_params[$tablename])) {
            ...
            if (empty($this->_tree_table_params[$tablename])) $this->_tree_table_params[$tablename] = array(array()); // empty means we searched already and found nothing
        }
    ...
    protected function walkTreeForRelationships($children, $parent_table_name, $parent_index_name, &$results_array, &$tableparams) {
		foreach($children as $child) {
			if (isset($child['heading'])) {
				...
				if (!isset($child['index']))                    $child['index'] = $this->getPrimaryIndexName($child['table']);
				if (!isset($child['parent_index_in_parent']))   $child['parent_index_in_parent'] = $parent_index_name;
				if (!isset($child['parent_index']))             $child['parent_index'] = $child['parent_index_in_parent'];
				if (!isset($child['parent_table']))             $child['parent_table'] = $parent_table_name;
				...
				if (isset($child['linkto_table'])) {
					if (!isset($child['linkto_index_in_parent']))   $child['linkto_index_in_parent'] = $this->getPrimaryIndexName($child['linkto_table']);
					if (!isset($child['linkto_index']))             $child['linkto_index'] = $child['linkto_index_in_parent'];
					if (!isset($child['linkto_calls_me']))          $child['linkto_calls_me'] = array();
					if (!isset($child['linkto_calls_me']['singular']))  $child['linkto_calls_me']['singular'] = ucwords('Linked as '.$this->getNiceTableName($child['table']));
					if (!isset($child['linkto_calls_me']['plural']))    $child['linkto_calls_me']['plural'] = ucwords('Links as '.$this->getNiceTableName($child['table']));
