...
class UrlCallRegistry {
	...
	public function setTarget($viewname=null, $filename=null, $params=null) {
		if (!empty($viewname)) $this->_target_url_array['view'] = $viewname;
		if (!empty($filename)) $this->_target_url_array['file'] = $filename;
		if (!empty($params)) $this->_target_url_array['params'] = $params;
		return $this;
	}
	...
	public function formatHandlerUrl($buttonname,$viewname, $filename, $params=array()) {
		...
		if (!isset($params[$buttonname])) $params[$buttonname] = '';
	...
	protected function getFilledUrlArrayWithDefaults($url_array) {
		$out = $url_array;
		if (!isset($out['view'])) $out['view'] = $this->_controller_action->params['action'];
		if (!isset($out['file'])) $out['file'] = $this->_controller_action->params['controller'];
		if (!isset($out['params'])) $out['params'] = array();
		...
		foreach($this->getPropagatingParamValues() as $fieldname => $value) {
			if (!isset($out['params'][$fieldname])) $out['params'][$fieldname] = $value;
		}
		return $out;
	}
	...
	public function getCurrentViewUrl($viewname=null, $filename=null, $params=null) {
		...
		if (($viewname!=null) || ($filename!=null) || ($params!=null)) {
			if (!empty($viewname)) $arr['view'] = $viewname;
			if (!empty($filename)) $arr['file'] = $filename;
			if (!empty($params)) $arr['params'] = $params;
		}
			...
			*/
			if ($match) {
				...
				if (preg_match('"^(http://|https://)"i',trim($path['return_url']))) {
					...
					if (!$return_uri->valid()) return $this->_last_chance_return_url;
...
function merge_in_post_vars($fields,$default_fields=array()) {
	...
	foreach ($default_fields as $name => $default_value) {
		if (!isset($_POST[$name])) $_POST[$name] = $default_value;
	}
...
function make_good_var_name($str, $maxlen = 40) {
	...
	if (strlen($stra)<=$maxlen) {
		...
		if (($goodending==0) || ($goodending===false)) $goodending = $maxlen;
...
function block_text_html($text) { // used for some showdialog messages
...
function detailActionToHtml($action_name,$detail_action) {
	...
	if ($action_name=='finish') $name_html = '<IMG style="vertical-align:middle;" src="'.$baseurl.'/images/flag_blue.png" width="16" height="16" border="0" alt="finish">';
	return $name_html;
}
...
function extractFieldOptions($optionss,$fieldname) {
	...
	if (!is_array($optionss)) $optionss = array('ALL' => $optionss);
