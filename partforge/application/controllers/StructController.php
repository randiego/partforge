...
class StructController extends DBControllerActionAbstract
{
        ...
        if (isset($this->params['form'])) {
            switch (true) {
                case isset($this->params['btnNewItem']):
                    ...
                    if (isset($this->params['partnumber_alias'])) {
                        $initialize['partnumber_alias'] = $this->params['partnumber_alias'];
                    }
        ...
        if (isset($this->params['form'])) {
            switch (true) {
                case isset($this->params['btnNewItem']):
                    ...
                    if (is_array($this->params['hidefields'])) {
                        $params['hidefields'] = $this->params['hidefields'];
                    }
    ...
    public function keepaliveAction()
    {
        ...
        if ($out['is_valid_user']) {
            $out['login_id'] = $_SESSION['account']->login_id;
        }
    ...
    */
    public function editviewdbAction()
    {
        ...
        if (in_array($this->view->version_edit_mode, array('vem_finish_save_record','vem_edit_version'))) {
            $this->view->edit_action_button = 'btnOK';
        }
        if ('vem_new_version'==$this->view->version_edit_mode) {
            $this->view->edit_action_button = 'btnChangePart';
        }
            ...
            if (isset($get_vars[$EditRow->getIndexName()])) {
                if ($EditRow->getRecordById($get_vars[$EditRow->getIndexName()])) {
                    ...
                    if (count($msgs)>0) {
                        ...
                        if (isset($this->params['return_url_failed'])) {
                            $return_to_url = $this->params['return_url_failed'];
                        }
