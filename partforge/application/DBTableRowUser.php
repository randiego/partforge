...
class DBTableRowUser extends DBTableRow {
    ...
    public function reasonsWhyCantRecieveWatchNotices()
    {
        $m = array();
        if (!$this->user_enabled) {
            $m[] = 'Account is not enabled.';
        }
        if ($this->waiting_approval) {
            $m[] = 'Account has not been approved yet.';
        }
        if (in_array($this->getRole(), array('Guest','DataTerminal'))) {
            $m[] = 'This user type is not allowed to receive watch notices.';
        }
        if (!$this->email) {
    ...
    public function save($fieldnames = array(), $handle_err_dups_too = true)
    {
        ...
        if (in_array('password', $fieldnames)) {
            ...
            unset($fieldnames[array_search('password', $fieldnames)]);
            if (!in_array('user_cryptpassword', $fieldnames)) {
                $fieldnames[] = 'user_cryptpassword';
            }
        }
...
return false;
    ...
    public function whoAmIHtml()
    {
        if (LoginStatus::getInstance()->isValidUser()) {
            ...
            if (LoginStatus::getInstance()->returnLoginExists()) {
                $html .= '<p>'.linkify(UrlCallRegistry::formatViewUrl('returnlogin', 'user'), 'Switch back to '.LoginStatus::getInstance()->getReturnLogin(), '', 'minibutton2 switch_user_button').'</p>';
            }
            $fieldtype = $this->getFieldType('user_type');
            $next_role = $this->getNextRole();
            $next_role_name = $fieldtype['options'][$next_role];
            if ($this->canRoleSwitch()) {
                $html .= '<p>'.linkify(UrlCallRegistry::formatViewUrl('switchrole', 'user'), 'Switch to '.$next_role_name.' View', '', 'minibutton2 switch_user_button').'</p>';
            }
        } else {
            $html = '';
        }
    ...
    public function setDataTerminalObjectIds($typeobject_ids)
    {
        ...
        if (count($ones_to_delete)>0) {
            DbSchema::getInstance()->mysqlQuery("delete from terminaltypeobject where (user_id='{$this->user_id}') and (allowed_typeobject_id IN (".implode(',', $ones_to_delete)."))");
        }
