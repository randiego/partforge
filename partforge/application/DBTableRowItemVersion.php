 ...
 */
    class DBTableRowItemVersion extends DBTableRow {
        ...
        public function assignFromFormSubmission($in_params,&$merge_params) {
        	...
        	if ($merge_params['typeversion_id']) $typeversion_id = $merge_params['typeversion_id'];
        	if (isset($in_params['typeversion_id'])) $typeversion_id = $in_params['typeversion_id'];
        ...
        private function tryInitializingComponentsByDrilling() {
        	...
        	*/
        	...
        	foreach($this->getCurrentlySetComponentValues() as $component_field_name => $itemobject_id) {
        		$ItemVersion = $this->_dbschema->dbTableRowObjectFactory('itemversion',false,'');
        ...
        */        
		...
		public function getExportFieldTypes() {
			...
			if ($this->hasASerialNumber()) $header_fields[] = 'item_serial_number';
			if ($this->hasADisposition()) $header_fields[] = 'disposition';
			if ($this->hasAliases()) {
        	...
        	if ($this->isExistingObject()) {
        		...
        		if (count($records)>0) {
        			...
        			if ((strtotime($this->effective_date)<strtotime($record['current_effective_date']))
        					&& ($this->itemversion_id != $record['itemversion_id'])) $saving_an_older_version = true;
        		}
        	}
        ...
        public function getItemsWithPropertyMatching($fieldname,$value) {
        	...
        	foreach($records as $itemversion_id => $record) {
        		...
        		if ($item_data[$fieldname]==$value) $out[$itemversion_id] = $record['item_serial_number'] ? $record['item_serial_number'] : "Item Version: {$itemversion_id}";
        	}
        	...
        	foreach($this->_typeversion_digest['components_in_defined_subfields'] as $component_name) {
				...
				*/
        		if (is_numeric($this->{$component_name})) {
					$OldComponent = $this->_dbschema->dbTableRowObjectFactory('itemversion',false,'');
  ...
  SELECT GROUP_CONCAT(DISTINCT typecomponent_typeobject.can_have_typeobject_id ORDER BY typecomponent_typeobject.can_have_typeobject_id SEPARATOR '|') FROM typecomponent_typeobject WHERE typecomponent_typeobject.typecomponent_id=typecomponent.typecomponent_id
        ...
        public function getJoinOptions($join_name,$include_only_orphans) {
        	...
        	if (!AdminSettings::getInstance()->use_any_typeversion_id) {
	        	if (!isset($this->_join_options[$join_name])) {
	                ...
	                $DbTableObj = DbSchema::getInstance()->dbTableRowObjectFactory('typeversion',true);
        ...
        public function getComponentSelectOptions($fieldname, $effective_date, $future_suffix_label=' (future effective date)', $only_self_ref_proc = true, $show_types=false) {
        	 ...
        	 */
        	$out = array();
        	$LLC = $this->_dbschema->dbTableRowObjectFactory('itemversion',false,'');
		...
		private function loadOrphanFieldsIntoHiddenArrays() {
			...
			if ($SavedRecord->getRecordById($this->itemversion_id)) {
				...
				if (!is_array($item_data)) $item_data = array(); // sometimes this field is not initialized
        ...
        function getEditViewFieldLayout($default_fieldnames,$parent_fields_to_remove,$layout_key=null) {
        	...
        	*/
        	...
        	if (!$this->isSaved() && !$this->previewDefinition() && ($layout_key=='editview')) $headerfields = array_diff($headerfields,array('typeversion_id'));
        	if ($layout_key=='editview') $headerfields = array_diff($headerfields,array('record_locator'));
        	if (!$this->hasASerialNumber()) $headerfields = array_diff($headerfields,array('item_serial_number'));
        	if (!$this->hasADisposition()) $headerfields = array_diff($headerfields,array('disposition'));
        	if (($layout_key!='editview') || !$this->hasAliases()) $headerfields = array_diff($headerfields,array('partnumber_alias'));
        	$header_layout = array();
