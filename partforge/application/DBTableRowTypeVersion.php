...
class DBTableRowTypeVersion extends DBTableRow {
    ...
    static public function typesListing()
    {
        ...
        $out = array(
            ...
            'calculated' => array('parameters' => array(
                    ...
                    'format' => array('type' => 'string', 'help' => 'Leave blank or enter format string: a value 1234.3456789 with "%.2f" outputs"1234.35", "%.4e" outputs "1.2343e+3", and "%.5g" outputs "1234.3" (5 dig total precision). For more, see docs for <a href="https://www.php.net/manual/en/function.sprintf.php" target="_blank">PHP sprintf function</a>.'),
                    'required' => array('type' => 'pickone', 'values' => array('0','1'), 'help' => '1 = warn user if they leave this field blank'),
    ...
    public function getRecordById($id)
    {
        ...
        $DBTableRowQuery->addSelectFields(array("CAST(
			 ...
			 IFNULL(typecomponent.show_new_btn,1)
            		)
    ...
    static public function groupConcatComponentsToFieldTypes($list_of_typecomponents, $calcCaption = true)
    {
        ...
        foreach (is_null($list_of_typecomponents) ? array() : explode(';', $list_of_typecomponents) as $typecomponent) {
            if (!empty($typecomponent)) {
                ...
                list($typecomponent_id, $component_name, $can_have_typeobject_id_packed, $caption, $subcaption, $featured, $required, $max_uses, $show_new_btn) = $component_parts;
                $can_have_typeobject_id = $can_have_typeobject_id_packed ? explode('|', $can_have_typeobject_id_packed) : array();
                $type_array = array('type' => 'component', 'can_have_typeobject_id' => $can_have_typeobject_id, 'subcaption' => '', 'required' => 0, 'featured' => 1, 'max_uses' => 1, 'show_new_btn' => 1); // superficially is like an enum or something.
                ...
                if (!is_null($featured)) {
                    $type_array['featured'] = $featured;
                }
                if (!is_null($required)) {
                    $type_array['required'] = $required;
                }
                if (!is_null($max_uses) && ($max_uses !== '')) {
                    $type_array['max_uses'] = $max_uses;
                }
                if (!is_null($show_new_btn) && ($show_new_btn !== '')) {
                    $type_array['show_new_btn'] = $show_new_btn;
                }
  ...
  (SELECT count(*) FROM itemversionarchive as iva WHERE iv.itemversion_id=iva.itemversion_id
    ...
    public function getWriteProtectedFieldnames($include_types_only = true)
    {
        ...
        if (!$include_types_only) {
            $out = array_unique(array_merge($out, array_keys(array_filter($this->getItemInstanceCounts()))));
        }
    ...
    static public function getTypeVersionDigestFromFields($list_of_typecomponents, $has_a_serial_number, $has_a_disposition, $all_typeversion_fields, $skip_components, $antirecursion_manifest)
    {
        ...
        foreach ($component_typeversions_to_load as $component_name => $typeobject_ids) {
            foreach ($typeobject_ids as $typeobject_id) {
                ...
                if (!$all_typeversion_fields['effective_date']) {
        ...
        foreach ($subfieldtype as $fieldname => $fieldtype_in_current_dictionary) {
            ...
            if (in_array($subfieldname, $tvd['addon_property_fields']) || in_array($subfieldname, $tvd['addon_component_subfields'])) {
                ...
                */
                ...
                if (!in_array($component_name, $out['components_in_defined_subfields'])) {
                    $out['components_in_defined_subfields'][] = $component_name;
                }
            }
        }
                ...
                if ($type=='columns') {
                    foreach ($row['columns'] as $field_index => $field) {
                        ...
                        if (in_array($name, $fields_to_remove)) {
                            ...
                            if (empty($fieldlayout_tbl[$row_index]['columns'])) {
                                unset($fieldlayout_tbl[$row_index]);    // if this was the last column for this row, remove the row too
                            }
                        }
                    }
                }
    ...
    */
    public function saveVersioned($user_id = null, $handle_err_dups_too = true)
    {
        ...
        */
        ...
        foreach ($component_types as $fieldname => $component_type) {
            ...
            $Comp->show_new_btn = $component_type['show_new_btn'];
            $Comp->save();
    ...
    static function saveOrRebuildPartNumberCache($typeversion_id, $type_part_number, $type_description)
    {
        ...
        $part_numbers = explode('|', (string) $type_part_number);
        $part_descriptions = explode('|', (string) $type_description);
  ...
  SELECT GROUP_CONCAT(DISTINCT typecomponent_typeobject.can_have_typeobject_id ORDER BY typecomponent_typeobject.can_have_typeobject_id SEPARATOR '|') FROM typecomponent_typeobject WHERE typecomponent_typeobject.typecomponent_id=typecomponent.typecomponent_id
) as can_have_typeobject_id, component_name, IFNULL(caption,'') as caption, IFNULL(subcaption,'') as subcaption, IFNULL(featured,0) as featured, IFNULL(required,0) as required, IFNULL(max_uses,1) as max_uses, IFNULL(show_new_btn,1) as show_new_btn FROM typecomponent WHERE belongs_to_typeversion_id='{$this->typeversion_id}'");
        ...
        foreach ($comp_records_to_delete as $typecomponent_id => $typecomponent) {
            ...
            if (isset($components_to_save[$component_name_on_disk])) {
                ...
                if (($typecomponent['can_have_typeobject_id']==$to_save['can_have_typeobject_id'])
                        ...
                        && ($typecomponent['max_uses']==$to_save['max_uses']) && ($typecomponent['show_new_btn']==$to_save['show_new_btn'])) {
    ...
    {
        $result = true;
        if (self::getNumberOfItemsForTypeVersion($this->typeversion_id)>0) {
            $result = false;
        }
    ...
    public function validateFields($fieldnames, &$errormsg)
    {
        if (!self::isTypeCategoryAProcedure($this->typecategory_id) && in_array('serial_number_type', $fieldnames) && !is_null($this->serial_number_type)) {
            ...
            foreach ($snfields as $field => $value) {
                if (in_array($field, $fieldnames)) {
                    unset($fieldnames[array_search($field, $fieldnames)]);
                }
            }
        ...
        if (in_array('type_part_number', $fieldnames)) {
            if (count(explode('|', (string) $this->type_part_number))!=count(array_unique(explode('|', (string) $this->type_part_number)))) {
                $errormsg[] = 'You have duplicate item numbers.  Please make all your part/procedure numbers different.';
            }
            foreach (explode('|', (string) $this->type_part_number) as $idx => $part_number) {
                if (!$part_number) {
        ...
        if (in_array('type_description', $fieldnames)) {
            foreach (explode('|', (string) $this->type_description) as $idx => $description) {
                if (!$description) {
    ...
    */
    ...
    public function canVersionBeUpgradedSafelyTo(self $TargetTypeVersion)
    {
        ...
        if (count($problem_fields)) {
            $fail_msg[] = 'There are fields ('.implode(', ', $problem_fields).') in this version that are not in the target version.';
        }
