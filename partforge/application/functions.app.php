...
function fetch_form_page($title,$html,$buttons,$bottom_html='',$on_click_array=array(),$attributes='') {
	if (!is_array($on_click_array)) $on_click_array=array();
	$btns = '';
...
function showdialog($errheader, $errormsg, $buttonlist) {   // display the message, then proceed
...
function showinputdialog($title, $message, $label, $text) {   // display the message, the proceed
...
function spawnshowdialog($title, $message, $buttonlist) {   // looks just like above, except spawned as new url
...
class BreadCrumbsManager {
	...
	public function render($current_url=null, $override_title=null, $max_entries=0, $divclass='breadcrumbdiv') {
		if (!is_null($current_url)) $this->_current_url = $current_url;
		$bc_links = array();
		foreach($_SESSION['breadcrumbs'] as $url => $url_name) {
		...
		if ($max_entries>0) $bc_links = array_slice($bc_links,-$max_entries);
...
function specialSearchKeyToUrl($search_string, $strict=true) {
	if (!$strict && ctype_digit($search_string)) $search_string = 'io/'.$search_string;
...
function event_log_notify_clear() {
	$Messages = new DBRecords(DbSchema::getInstance()->dbTableRowObjectFactory('eventlog',false),'','');
