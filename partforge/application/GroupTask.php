...
class GroupTask {
    ...
    public function getRespondedUserNames()
    {
        ...
        foreach ($records as $assigned_to_task_id => $record) {
            if ($record['responded_on']) {
                $names[] = DBTableRowUser::getFullName($record['user_id']);
            }
        }
     ...
     */
    static public function getLinkUrlForMember($assigned_to_task_id)
    {
