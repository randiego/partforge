...
class ImportStrategyObjects {
    ...
    */
    	...
    	*/
    	if ($action=='NewObject') {
    		if ($typeversion_id) {
    			...
    			if ($user_id) $ItemVersion->user_id = $user_id;
    		...
    		*/
    		...
    		if ($itemversion_id) {
    			if ($ItemVersion->getRecordById($itemversion_id)) {
                    ...
                    */
	    			if ($typeversion_id) $ItemVersion->typeversion_id = $typeversion_id;
    				if ($effective_date) $ItemVersion->effective_date = $effective_date;
    				foreach($curr_field_to_columns as $fieldname => $columnname) {
    					$type = $ItemVersion->getFieldType($fieldname);
    ...
    static function storeObjectsFromArray($importParams, $simlulate_only=false) {
    	if (count($importParams['records'])==0) return array();
    	...
    	foreach($importParams['column_defs'] as $columnname => $fieldname) {
    		...
    		if (isset($first_row[$columnname])) $field_to_columns[$fieldname] = $columnname;
    	}
    	...
    	foreach($importParams['records'] as $idx => $record) {
    		...
    		*/
    		$action = 'NewObject';
    		if (isset($curr_field_to_columns['IMPORT_ACTION']) && isset($record[$curr_field_to_columns['IMPORT_ACTION']])) $action = $record[$curr_field_to_columns['IMPORT_ACTION']];
    		unset($curr_field_to_columns['IMPORT_ACTION']);
    
    		$typeversion_id = null;
    		// don't use the selection box input if this is a 'ReplaceFields' action.
    		if (($action!='ReplaceFields') && isset($importParams['typeversion_id'])) $typeversion_id = $importParams['typeversion_id'];
    		if (isset($curr_field_to_columns['typeversion_id']) && is_numeric($record[$curr_field_to_columns['typeversion_id']])) $typeversion_id = $record[$curr_field_to_columns['typeversion_id']];
