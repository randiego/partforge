...
*/
    protected function formatSortKeyName($field)
    {
        ...
        if (count($line)>1) {
            ...
            // this is meant to extract a normal name after some punctuation so mydomain.m_field => m_field
    ...
    public function display_fields(UrlCallRegistry $navigator, $queryvars, $get_legend_only = false, $query_key_name = 'btnChangeSortKey', $readonly = false)
    {
            ...
            if ($fielddesc['display']) {
                ...
                if (isset($fielddesc['key_asc'])) {
                    ...
                    if (isset($fielddesc['sort_key_fixed']) && $fielddesc['sort_key_fixed']) {
                        ...
                        if (isset($new_sort_array[1]) && in_array($new_sort_array[1], $colkeys1and2)) {
                            unset($new_sort_array[1]);
                        }
                        if (isset($new_sort_array[2]) && in_array($new_sort_array[2], $colkeys1and2)) {
                            unset($new_sort_array[2]);
                        }
        ...
        if ($get_legend_only) {
            ...
            foreach ($legend_items as $legend_index => $legend_text) {
                if (!isset($legend_array[$legend_index])) {
                    unset($legend_items[$legend_index]);
                }
            }
            if (count($legend_items)==1) {
                $legend_items = array();
            }
            return $legend_items;
        } else {
