...
class WatchListReporter {
    ...
    static public function processCurrentDailyWatchNotifications()
    {
        ...
        foreach ($user_ids_records as $user_ids_record) {
            ...
            if ($User->getRecordById($user_id)) {
                ...
                if (is_null($user_blocks_notifications)) {
                    $user_blocks_notifications = false;
                }
    ...
    {
        $ItemVersionForTypeInfo = new DBTableRowItemVersion();
        $html = '';
        ...
        foreach ($records as $record) {
            ...
            if ($loc_parts[0]=='tv') {
                $item_name = linkify(formatAbsoluteLocatorUrl($loc_parts[0], $loc_parts[1]), $item_name);
            }
            ...
            $item_serial_number = '';
            if (!is_null($record['item_serial_number'])) {
                $item_serial_number = linkify(formatAbsoluteLocatorUrl($loc_parts[0], $loc_parts[1]), TextToHtml($record['item_serial_number']));
            }

            $item_disposition = '';
            if (!is_null($record['procedure_disposition'])) {
                $item_disposition = ' ('.DBTableRowItemVersion::renderDisposition($ItemVersionForTypeInfo->getFieldType('disposition'), $record['procedure_disposition'], false, '').')';
            }
