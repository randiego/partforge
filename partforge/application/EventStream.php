    ...
    if (strtotime($a['effective_date']) < strtotime($b['effective_date'])) {
        ...
        if (strtotime($a['record_created']) < strtotime($b['record_created'])) {
            ...
            if ($typeord[$a['event_type_id']] < $typeord[$b['event_type_id']]) {
                ...
                if (!isset($a['indented_component_name']) && isset($b['indented_component_name'])) {
                    $result = 1;
                } elseif (isset($a['indented_component_name']) && !isset($b['indented_component_name'])) {
                    $result = -1;
                }
            }
        }
...
class EventStream {
    ...
    static public function getNestedEventStreamRecords(DBTableRowItemVersion $ItemVersion, $end_date = null)
    {
        ...
        if (!$ItemVersion->is_user_procedure) {
            ...
            foreach ($streamlines as $idx => $streamline) {
                if ( in_array($streamline['event_type_id'], array('ET_CHG','ET_PROCREF','ET_PARTREF'))) {
                    $parent_itemversion_ids[] = $streamline['this_itemversion_id'];
                }
            }
    ...
    public static function textToHtmlWithEmbeddedCodes($event_description, $navigator, $event_type, $description_is_html, $genHtmlForPdf)
    {
        ...
        if (!$description_is_html) {
            $event_description = self::embeddedLocatorsToItemVersionTags($event_description);
        }
        if (!$description_is_html) {
    ...
    public static function eventStreamRecordsToLines($records, $dbtable, $navigator, $genHtmlForPdf)
    {
        ...
        foreach ($records as $record) {
            if (($record['event_type_id']=='ET_CHG') && (!isset($record['indented_component_name']))) {
                $version_count++;
            }
        }
        if ($version_count==1) {
        ...
        foreach ($records as $record) {
            ...
            if (in_array($record['event_type_id'], array('ET_PROCREF','ET_PARTREF'))) {
                ...
                if (isset($record['actual_effective_date'])) {
                    $line['actual_effective_date'] = $record['actual_effective_date'];
                }
                ...
                if (empty($record['indented_component_name'])) {
                    if (!isset($references_by_typeobject_id[$record['this_typeobject_id']])) {
                        $references_by_typeobject_id[$record['this_typeobject_id']] = array();
                    }
    ...
    static public function renderDashboardView($dbtable, $procedure_records, $references_by_typeobject_id, $more_to_show_msg_html, $showlinks)
    {
        ...
        foreach ($procedure_records_by_typeobject_id as $typeobject_id => $procedure_record) {
            ...
            $link = '';
            // We should only make an Add button when this is not obsolete
            if ($procedure_record['typedisposition'] != 'B') {
