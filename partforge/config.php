...
$config['support_email'] = 'administrator@example.com';  //INSTALL:PARAM:SUPPORT_EMAIL:

// email address of person system error messages are sent to.
$config['webmaster_email'] = 'administrator@example.com';  //INSTALL:PARAM:WEBMASTER_EMAIL:

// address from which workflow notifications are sent.
$config['notices_from_email'] = 'administrator@example.com';  //INSTALL:PARAM:NOTICES_FROM_EMAIL:
...
$config['document_path_base'] = 'C:\wamp\www\partforge\partforge';   //INSTALL:PARAM:DOCUMENT_PATH_BASE:

// the absolute file path of the reports folder.  For example: '/var/www/partforge/reports' or 'c:/wamp/www/partforge/reports'
$config['reports_classes_path'] = 'C:\wamp\www\partforge\partforge\reports';   //INSTALL:PARAM:REPORTS_CLASSES_PATH:
...
// add the following lines for automated testing.
$config['config_for_testing'] = isset($_SERVER['HTTP_USER_AGENT']) && ($_SERVER['HTTP_USER_AGENT']=='SimpleTest/1.0.0.0');
if ($config['config_for_testing']) {
	// the absolute path to the tester config file.
	require_once("C:/wamp/www/qdforms2/tests/config.php");
}

error_reporting(-1); // this is to make sure we always see the errors first during testing.
